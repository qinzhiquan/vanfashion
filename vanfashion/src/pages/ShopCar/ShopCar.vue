      ...
      <div class="pro_box" v-else>购物车里空空如也</div>
		...
		<div id="footer">
			...
			<div class="foot_total">
				...
				<div class="btn-area">
					<a class="btn-sumbit" @click="addOrder" :class="{'btn-allow': totalAmount}">结&nbsp;算</a>
				</div>
			</div>
		</div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      async addOrder(goods){
        ...
        let checkGoods = this.cartgoods.filter(item=>{
          return item.checked
        })
        console.log(checkGoods);
        let allPrice = checkGoods.reduce((pre,cur)=>{
          return pre + cur.price * cur.buy_count
        }, 0)

        // 1. 发送请求
        // user_id, goods_id, goods_name, thumb_url, price, buy_count, counts
        if(this.userInfo.user_name){
          let result = await addOrder(this.userInfo.id, this.userInfo.user_name, 1, allPrice, checkGoods);
